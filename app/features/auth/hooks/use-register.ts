...
import { authControllerRegister, authControllerSendOtp, authControllerVerifyEmail } from '~/api/operations/auth/auth'
import { extractApiErrorMessage } from '~/shared/lib/api/extract-api-error'

import type { RegisterBodyDto, SendOtpBodyDto, VerifyEmailBodyDto } from '~/api/model/auth'

type UseRegisterResult = {
  /** Gửi email đăng ký để BE tạo tài khoản (chưa activate). */
  register: (payload: RegisterBodyDto) => Promise<boolean>
...
export function useRegister(): UseRegisterResult {
  ...
  const verifyEmail = useCallback(
    async (payload: VerifyEmailBodyDto) => {
      setIsVerifying(true)
      try {
