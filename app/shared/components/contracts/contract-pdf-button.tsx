...
export function ContractPdfButton({
  ...
  const download = async () => {
    ...
    try {
      ...
      target?.close()
      toast.error(extractApiErrorMessage(error, t('contractShared.pdf.error')))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className='flex max-w-sm flex-col items-end gap-1.5'>
      <button
        ...
        title={conditionsMissing ? t('contractShared.pdf.missingConditionsTitle') : undefined}
        className={cn(
      ...
      >
        ...
        {isLoading ? t('contractShared.pdf.preparing') : t('contractShared.pdf.download')}
      </button>
      {conditionsMissing && (
        <p className='inline-flex items-center gap-1 text-right text-xs font-semibold text-destructive'>
          <AlertTriangle className='size-3.5 shrink-0' />
          {t('contractShared.pdf.missingConditions')}
        </p>
      )}
    </div>
  )
