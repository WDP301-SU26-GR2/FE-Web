...
function SeriesPanel({ series, allSeries, search, status, setSearch, setStatus, localize, t }: { series: SeriesListResDtoOutputItemsItem[]; allSeries: SeriesListResDtoOutputItemsItem[]; search: string; status: string; setSearch: (value: string) => void; setStatus: (value: string) => void; localize: (group: string, value: string | null | undefined) => string; t: TFunction }) {
  return <Panel title={t('operations.monitoring.series')} description={t('operations.monitoring.seriesDescription', { defaultValue: 'Theo dõi trạng thái phát hành của toàn bộ bộ truyện.' })}>
    ...
    <div className='mt-4 space-y-2'>{series.slice(0, 12).map((item) => <article key={item.id} className='flex items-center justify-between gap-3 rounded-lg border border-border bg-background/50 px-3 py-3'><div className='min-w-0'><p className='truncate text-xs font-bold text-foreground'>{item.title}</p><p className='mt-1 text-[11px] text-muted-foreground'>{localize('seriesStatuses', item.status)}</p></div><span className='shrink-0 rounded-full bg-muted px-2 py-1 text-[10px] font-bold text-muted-foreground'>{item.id.slice(0, 8)}</span></article>)}{!series.length && <EmptyState text={t('operations.monitoring.empty')} />}</div>
