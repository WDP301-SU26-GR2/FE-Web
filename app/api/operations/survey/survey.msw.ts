...
export const getSurveyControllerGetSurveyPeriodVotesResponseMock = (): ReaderVoteResDtoOutput[] => (Array.from({ length: faker.number.int({ min: 1, max: 10 }) }, (_, i) => i + 1).map(() => ({id: faker.string.alpha({length: {min: 10, max: 20}}), surveyPeriodId: faker.string.alpha({length: {min: 10, max: 20}}), seriesIds: Array.from({ length: faker.number.int({ min: 1, max: 10 }) }, (_, i) => i + 1).map(() => (faker.string.alpha({length: {min: 10, max: 20}}))), identityHash: faker.helpers.arrayElement([faker.string.alpha({length: {min: 10, max: 20}}), null]), publicationType: faker.helpers.arrayElement([faker.helpers.arrayElement(['WEEKLY','MONTHLY','IRREGULAR'] as const), null]), authMethod: faker.helpers.arrayElement([faker.helpers.arrayElement(['EMAIL_OTP','PHONE_OTP','CAPTCHA_ONLY'] as const), null]), ipHash: faker.helpers.arrayElement([faker.string.alpha({length: {min: 10, max: 20}}), null]), captchaScore: faker.helpers.arrayElement([faker.number.float({min: undefined, max: undefined, fractionDigits: 2}), null]), voteWeight: faker.number.float({min: undefined, max: undefined, fractionDigits: 2}), isFlagged: faker.datatype.boolean(), votedAt: `${faker.date.past().toISOString().split('.')[0]}Z`})))
