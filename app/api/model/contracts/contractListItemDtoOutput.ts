/**
 * Generated by orval v7.21.0 🍺
 * Do not edit manually.
 * WDP API
 * Mangaka Backend API.

### ⚠️ Response envelope (ĐỌC TRƯỚC)
Mọi response **thành công** đều được bọc envelope — schema/Example Value bên dưới mô tả phần **CHƯA bọc** (chính là `data`):
```jsonc
{ "success": true, "message": "Thành công", "data": { /* shape mô tả trong từng API *\/ } }
```
→ **FE luôn đọc `res.data`** (KHÔNG đọc thẳng field gốc). Một số API trả `message` tuỳ biến (vd xoá) → message nằm ở top-level, `data` có thể `null`.

Mọi response **lỗi** (chuẩn hoá bởi 1 filter duy nhất):
```jsonc
{ "success": false, "statusCode": 409, "code": "Error.ProposalNotEditable",
  "message": "Không thể chỉnh sửa bản đề xuất ở trạng thái hiện tại" }             // lỗi đơn
{ "success": false, "statusCode": 422, "code": "Error.ValidationFailed",
  "message": "Địa chỉ email không hợp lệ",
  "errors": [ { "code": null, "message": "Địa chỉ email không hợp lệ", "path": "email" } ] } // lỗi field-level
```
`message` luôn là tiếng Việt để hiển thị; FE phân nhánh theo `code` ổn định. Validation fail = **422** (không phải 400).
 * OpenAPI spec version: 1.0
 */
import type { ContractListItemDtoOutputSeries } from './contractListItemDtoOutputSeries';
import type { ContractListItemDtoOutputMangaka } from './contractListItemDtoOutputMangaka';
import type { ContractListItemDtoOutputEditor } from './contractListItemDtoOutputEditor';
import type { ContractListItemDtoOutputContractType } from './contractListItemDtoOutputContractType';
import type { ContractListItemDtoOutputStatus } from './contractListItemDtoOutputStatus';
import type { ContractListItemDtoOutputRepresentative } from './contractListItemDtoOutputRepresentative';

export interface ContractListItemDtoOutput {
  id: string;
  seriesId: string;
  mangakaId: string;
  /** @nullable */
  editorId: string | null;
  /** Thông tin hiển thị — CÓ ở GET list/detail */
  series?: ContractListItemDtoOutputSeries;
  /** Thông tin hiển thị — CÓ ở GET list/detail */
  mangaka?: ContractListItemDtoOutputMangaka;
  /**
   * null = chưa gán; absent ở mutation path
   * @nullable
   */
  editor?: ContractListItemDtoOutputEditor;
  /** @nullable */
  boardDecisionId: string | null;
  /** Loại hợp đồng: FULL_BUYOUT (NXB mua đứt 100%, toàn quyền) | REVENUE_SHARE (ăn chia %, quyết định lớn cần Mangaka đồng ý) — BR-CONTRACT-03. Values: FULL_BUYOUT, REVENUE_SHARE */
  contractType: ContractListItemDtoOutputContractType;
  /** @nullable */
  valuationAmount: number | null;
  /** @nullable */
  publisherOwnershipPct: number | null;
  /** @nullable */
  mangakaOwnershipPct: number | null;
  /**
   * ISO 8601 date-time (UTC)
   * @nullable
   * @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z|([+-](?:[01]\d|2[0-3]):[0-5]\d)))$
   */
  contractStart: string | null;
  /**
   * ISO 8601 date-time (UTC)
   * @nullable
   * @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z|([+-](?:[01]\d|2[0-3]):[0-5]\d)))$
   */
  contractEnd: string | null;
  /** DRAFT: Editor đang soạn; có thể cập nhật điều khoản và payment conditions. | BOARD_REVIEW: Editor đã submit; chờ một Board representative trong roster claim và ký. | AWAITING_MANGAKA: Board representative đã ký; chờ Mangaka chấp nhận/ký hoặc từ chối. | ACTIVATION_PENDING: Replacement contract đã đủ chữ ký; chờ transfer finalizer kích hoạt và terminate hợp đồng cũ. | FULLY_EXECUTED: Hợp đồng đã đủ chữ ký và có hiệu lực. | REJECTED_BY_MANGAKA: Mangaka từ chối sau bước Board representative ký; Editor có thể redraft. | FULFILLED: Hợp đồng đã hoàn thành nghĩa vụ. | TERMINATED: Hợp đồng đã chấm dứt. | TERMINATED_BY_BREACH: Hợp đồng chấm dứt do vi phạm. | EXPIRED: Hợp đồng hết hạn. | VOIDED: Hợp đồng nháp/đang ký đã bị vô hiệu hoá.. Values: DRAFT, BOARD_REVIEW, AWAITING_MANGAKA, ACTIVATION_PENDING, FULLY_EXECUTED, REJECTED_BY_MANGAKA, FULFILLED, TERMINATED, TERMINATED_BY_BREACH, EXPIRED, VOIDED */
  status: ContractListItemDtoOutputStatus;
  /** @nullable */
  representativeId?: string | null;
  /**
   * Đại diện Hội đồng đã claim/gán; absent ở mutation path
   * @nullable
   */
  representative?: ContractListItemDtoOutputRepresentative;
  /** @nullable */
  supersedesContractId?: string | null;
  /**
   * ISO 8601 date-time (UTC)
   * @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z|([+-](?:[01]\d|2[0-3]):[0-5]\d)))$
   */
  createdAt: string;
}
