 ...
 */
import type { AmendmentResDtoOutputStatus } from './amendmentResDtoOutputStatus';
import type { AmendmentResDtoOutputTriggerSource } from './amendmentResDtoOutputTriggerSource';
import type { AmendmentResDtoOutputCreator } from './amendmentResDtoOutputCreator';
import type { AmendmentResDtoOutputSignaturesItem } from './amendmentResDtoOutputSignaturesItem';

export interface AmendmentResDtoOutput {
  id: string;
  contractId: string;
  changedClauses: string[];
  /** @nullable */
  reason: string | null;
  /** Vòng đời phụ lục hợp đồng: DRAFT → PENDING_SIGNATURES → FULLY_EXECUTED | VOIDED (reject → về DRAFT). Values: DRAFT, PENDING_SIGNATURES, FULLY_EXECUTED, VOIDED */
  status: AmendmentResDtoOutputStatus;
  /** Nguồn phát sinh phụ lục: MANUAL (Editor tự tạo) | FORMAT_CHANGE | COMPLETION (từ quyết định Flow 5 — BR-CONTRACT-06). Values: MANUAL, FORMAT_CHANGE, COMPLETION */
  triggerSource: AmendmentResDtoOutputTriggerSource;
  /** @nullable */
  ...
  /** @nullable */
  creator?: AmendmentResDtoOutputCreator;
  /**
   * ISO 8601 date-time (UTC)
   * @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z|([+-](?:[01]\d|2[0-3]):[0-5]\d)))$
   */
  createdAt: string;
  signatures?: AmendmentResDtoOutputSignaturesItem[];
}
