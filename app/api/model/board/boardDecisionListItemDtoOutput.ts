/**
 * Generated by orval v7.21.0 🍺
 * Do not edit manually.
 * WDP API
 * Mangaka Backend API.

### ⚠️ Response envelope (ĐỌC TRƯỚC)
Mọi response **thành công** đều được bọc envelope — schema/Example Value bên dưới mô tả phần **CHƯA bọc** (chính là `data`):
```jsonc
{ "success": true, "message": "Thành công", "data": { /* shape mô tả trong từng API *\/ } }
```
→ **FE luôn đọc `res.data`** (KHÔNG đọc thẳng field gốc). Một số API trả `message` tuỳ biến (vd xoá) → message nằm ở top-level, `data` có thể `null`.

Mọi response **lỗi** (chuẩn hoá bởi 1 filter duy nhất):
```jsonc
{ "success": false, "statusCode": 409, "code": "Error.ProposalNotEditable",
  "message": "Không thể chỉnh sửa bản đề xuất ở trạng thái hiện tại" }             // lỗi đơn
{ "success": false, "statusCode": 422, "code": "Error.ValidationFailed",
  "message": "Địa chỉ email không hợp lệ",
  "errors": [ { "code": null, "message": "Địa chỉ email không hợp lệ", "path": "email" } ] } // lỗi field-level
```
`message` luôn là tiếng Việt để hiển thị; FE phân nhánh theo `code` ổn định. Validation fail = **422** (không phải 400).
 * OpenAPI spec version: 1.0
 */
import type { BoardDecisionListItemDtoOutputTargetSeries } from './boardDecisionListItemDtoOutputTargetSeries';
import type { BoardDecisionListItemDtoOutputDecisionType } from './boardDecisionListItemDtoOutputDecisionType';
import type { BoardDecisionListItemDtoOutputResult } from './boardDecisionListItemDtoOutputResult';

export interface BoardDecisionListItemDtoOutput {
  id: string;
  /** @nullable */
  targetSeriesId?: string | null;
  /**
   * CÓ ở GET /board/decisions + /:id (enrich); null nếu decision không gắn series
   * @nullable
   */
  targetSeries?: BoardDecisionListItemDtoOutputTargetSeries;
  boardSessionId: string;
  /**
   * Loại quyết định Hội đồng: SERIALIZATION (duyệt serial hoá — chỉ khi series PITCHED) | CANCELLATION (huỷ bộ truyện) | COMPLETION (duyệt kết thúc) | FORMAT_CHANGE (đổi hình thức xuất bản) | REPRINT (tái bản) | TRANSFER (chuyển nhượng) | CONTRACT (duyệt hợp đồng/phụ lục) | SERIES_CONTRACT_APPROVAL (nội bộ, không tạo qua API). Ghi nhận "giữ nguyên bộ truyện" = mở CANCELLATION rồi Hội đồng bỏ phiếu REJECT.. Values: SERIES_CONTRACT_APPROVAL, SERIALIZATION, CANCELLATION, FORMAT_CHANGE, COMPLETION, REPRINT, TRANSFER, CONTRACT
   * @nullable
   */
  decisionType?: BoardDecisionListItemDtoOutputDecisionType;
  /**
   * Kết quả quyết định Hội đồng: PENDING (đang bỏ phiếu), PENDING_QUORUM (chưa đủ quorum), APPROVED (thông qua), REJECTED (bác bỏ), EXPIRED (phiên đóng khi chưa chốt → cần mở phiên mới). Values: PENDING, PENDING_QUORUM, APPROVED, REJECTED, EXPIRED
   * @nullable
   */
  result?: BoardDecisionListItemDtoOutputResult;
  totalVotes: number;
  approveCount: number;
  rejectCount: number;
  quorumMet: boolean;
  /** @nullable */
  endingChapterAllowance?: number | null;
  /**
   * ISO 8601 date-time (UTC)
   * @nullable
   * @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z|([+-](?:[01]\d|2[0-3]):[0-5]\d)))$
   */
  decidedAt?: string | null;
  /**
   * ISO 8601 date-time (UTC)
   * @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z|([+-](?:[01]\d|2[0-3]):[0-5]\d)))$
   */
  createdAt?: string;
}
