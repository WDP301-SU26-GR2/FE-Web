...
**`BoardSessionPhase`** (read-only trừ route chuyển phase): `PRESENTING` → `QA` → `VOTING`. Phase bắt buộc đi đúng từng bước, không được giữ nguyên, nhảy bước hoặc đi lùi; chat bị khóa khi đã vào `VOTING`. Phiên `ACTIVE` chỉ nhận phiếu khi phase hiện tại là `VOTING`.
...
7. **Editor** `POST /board/sessions` với `seriesId` → hệ thống tự chọn ngẫu nhiên roster theo sĩ số đề xuất trong `BoardConfig.quorumMin`; client không được gửi danh sách hoặc sĩ số để tránh can thiệp thành phần biểu quyết. Danh tính thành viên chỉ nên hiển thị khi người dùng mở chi tiết phòng họp. Phiên tự `ACTIVE` khi tới `startTime` (hoặc `PATCH /board/sessions/:id/start`) và sinh ra ở **phase `PRESENTING`**.
...
9. **🆕 PHÒNG HỌP (Spec 16) — 3 giai đoạn, creator điều khiển:** mọi người trong roster join room WS `/board` (`joinSession` — rejoin lại được vô hạn khi rớt mạng, resync bằng `GET /board/sessions/:id` + `GET .../messages` + `GET /board/decisions?boardSessionId=`):
   ...
   - **Creator** `PATCH /board/sessions/:id/phase {phase:'QA'}` → **Q&A**: chat qua WS `sendMessage` → mọi người nhận `messageReceived` (lịch sử lưu DB). Không được bỏ qua giai đoạn này.
   - **Creator** `PATCH .../phase {phase:'VOTING'}` → room nhận `phaseChanged`; **từ đây CHAT BỊ KHÓA** (DENIED `VOTING_PHASE`) — chỉ vote.
10. **Board members** `POST /board/decisions/:id/vote` (APPROVE/REJECT/ABSTAIN — ⚠ **chỉ được khi phase=`VOTING`**, sớm hơn → 409 `Error.VotingNotOpen`; realtime tiến độ qua `voteProgressUpdated`). **Mọi thành viên trong roster phải bỏ phiếu** trước khi decision được chốt, để không bỏ sót ý kiến. Trong lúc chờ: chưa đủ `ceil(2/3 × roster)` phiếu → `PENDING_QUORUM`, đã đủ ngưỡng này nhưng chưa đủ toàn bộ roster → `PENDING`. Khi toàn bộ roster đã vote, **APPROVE > `approveMajorityRatio × roster`** → `APPROVED`, ngược lại → `REJECTED`. Decision `APPROVED` tự chuyển Series `SERIALIZED`, set slot từ `details`, thông báo Mangaka/Editor và mở Flow 6. Vote lại decision đã chốt → 409 `Error.DecisionAlreadyFinalized`.
11. Sau khi toàn bộ roster vote, phase vẫn giữ `VOTING`; creator chỉ thấy nút kết thúc khi mọi decision đã terminal. Creator gọi `PATCH /board/sessions/:id/conclude` (hoặc cron auto khi quá `endTime`) → session `CONCLUDED`. Decision còn treo → `EXPIRED`, Series giữ `PITCHED` — Editor mở phiên mới + decision mới (KHÔNG cần pitch lại; report cũ tra qua `?seriesId=`).
...
| 🆕 Đổi phase giữ nguyên, đi lùi hoặc nhảy bước | `PATCH /board/sessions/:id/phase` | 409 `Error.InvalidPhaseTransition` |
...
#### `POST /board/sessions`
> Editor tạo phiên họp Hội đồng → UPCOMING; hệ thống tự chọn ngẫu nhiên thành viên
...
| `seriesId` | string (regex) ✍ | ✅ | Series làm ngữ cảnh cho phiên. Server tự chọn ngẫu nhiên roster theo sĩ số đề xuất trong `BoardConfig.quorumMin`; client không được truyền danh sách/sĩ số thành viên. |
...
| 422 | `Error.RosterSourceRequired` | thiếu `seriesId` để hệ thống tự chọn roster |
| 422 | `Error.NotEnoughBoardMembers` | fewer than 3 active board members exist — cannot form a valid session |
| 422 | — | gửi `allowedEditorIds`, `rosterSize` hoặc field lạ khác; schema strict không cho client can thiệp roster |
...
> Creator hoặc Super Admin chuyển phần của phiên họp đúng từng bước: `PRESENTING → QA → VOTING` (không được bỏ qua QA)
...
| Field | Kiểu | Bắt buộc | Mô tả |
|---|---|---|---|
| `phase` | enum `BoardSessionPhase` | ✅ | Phase kế tiếp: `QA` sau `PRESENTING`, hoặc `VOTING` sau `QA`; không được giữ nguyên, nhảy bước hoặc đi lùi |
...
| 409 | `Error.InvalidPhaseTransition` | phase đích không phải bước kế tiếp ngay sau phase hiện tại |
...
| `quorumMin` | number | ✅ | Sĩ số roster đề xuất khi hệ thống tự chọn thành viên. Ngưỡng `ceil(2/3 × roster)` chỉ dùng hiển thị tiến độ `PENDING_QUORUM`; decision vẫn phải chờ toàn bộ roster vote mới chốt. |
...
| `quorumMin` | number ✍ | ✅ | Sĩ số roster đề xuất khi hệ thống tự chọn thành viên (≤ `boardTotalMembers`); không phải số phiếu đủ để chốt decision |
...
1. **Editor** chọn quyết định `SERIALIZATION` đã `APPROVED`; FE hiển thị tên series, phiên họp và ngày phê duyệt, sau đó tự lấy `boardDecisionId`, `seriesId`, `mangakaId` từ lựa chọn này để gọi `POST /contracts` (contractType + valuationAmount + ownership % + terminationClause) → `DRAFT`. Không yêu cầu người dùng nhập hoặc đối chiếu ID kỹ thuật. Chỉ tạo được khi series `SERIALIZED`.
...
| `boardDecisionId` | string (1..∞ ký tự) ✍ | ✅ | Quyết định `SERIALIZATION` đã `APPROVED` làm căn cứ; FE lấy từ lựa chọn có hiển thị series/phiên/ngày duyệt, không cho nhập ID tự do |
...
| `id` | string | ✅ | ObjectId của bản ghi |
| `seriesId` | string | ✅ | ObjectId của Series |
| `series` | object | ✅ | `{id, title}` của bộ truyện — dùng để hiển thị, không hiện `seriesId` kỹ thuật |
| `mangakaId` | string | ✅ | ObjectId User của Mangaka chủ sở hữu |
| `editorId` | string | ✅ | ObjectId User của Editor phụ trách |
| `boardDecisionId` | string | ✅ |  |
| `boardDecision` | object | — | Căn cứ quyết định Hội đồng; `null` nếu hợp đồng không gắn decision |
| `boardDecision.id` | string | ✅ | ObjectId quyết định; chỉ dùng để deep-link |
| `boardDecision.decisionType` | enum `DecisionType` | — | Với hợp đồng serial hóa thường là `SERIALIZATION` |
| `boardDecision.result` | enum `BoardDecisionResult` | — | Kết quả quyết định, thường là `APPROVED` |
| `boardDecision.decidedAt` | string (ISO 8601) | — | Thời điểm quyết định được chốt |
| `boardDecision.boardSession` | object | ✅ | `{id, title, startTime}` của phiên họp nguồn |
| `sourceTransferRequestId` | string | — |  |
| `contractType` | enum `ContractType` | ✅ | Loại hợp đồng: FULL_BUYOUT (NXB mua đứt 100%, toàn quyền) \| REVENUE_SHARE (ăn chia %, quyết định lớn cần Mangaka đồng ý) — BR-CONTRACT-03 |
| `valuationAmount` | number | ✅ | Giá trị định giá tác phẩm (Board quyết) |
| `publisherOwnershipPct` | number | ✅ | % sở hữu của NXB |
...
- `POST /board/sessions` bắt buộc `seriesId`; server tự chọn ngẫu nhiên roster theo cấu hình và không nhận roster/sĩ số do client chỉ định. `GET /board/suggest-members?seriesId=` chỉ là endpoint tham khảo, không quyết định roster thật của phiên.
...
- **Spec 17 (Board quorum & vote hardening):** decision chỉ chốt sau khi **toàn bộ roster đã vote**; khi đó APPROVE phải > `approveMajorityRatio × roster`, ngược lại REJECTED. Trước khi đủ toàn bộ roster, `PENDING_QUORUM`/`PENDING` chỉ thể hiện tiến độ. Vote lại decision đã chốt → **409 `Error.DecisionAlreadyFinalized`**. `GET /board/decisions` thêm query `targetSeriesId`; `GET /board/config` lazy-seed; `BoardConfig.quorumMin` là sĩ số roster mặc định.
...
1. **Luật chốt vote đổi (hành vi):** decision chỉ chuyển `APPROVED`/`REJECTED` sau khi **mọi thành viên roster đã bỏ phiếu**. Trước đó, dưới `ceil(2/3 × roster)` phiếu là `PENDING_QUORUM`; từ ngưỡng này đến trước phiếu cuối là `PENDING`. Sau phiếu cuối, APPROVE phải **> `approveMajorityRatio × roster`** (default 0.5) để được duyệt; ngược lại bị từ chối. Nhờ vậy thành viên cuối luôn được ghi nhận ý kiến, kể cả khi kết quả số học đã có thể dự đoán sớm.
