...
## 0. Tổng quan phạm vi — 78 route độc quyền BOARD_MEMBER
...
## 7. Đối chiếu hoàn thiện — 78/78 route

Đã viết đủ **78/78 route** BOARD_MEMBER theo `route-roles.ts` (không route nào bỏ sót). 7 route production-reference (`/chapters/:id/storyboards`, `/storyboards/:id`, `/chapters/:id/pages`, `/chapters/:id/progress`, `/chapters/:id/stages`, `/revision-requests`, `/tasks/:id/download-url`) được mô tả ở §6.8.
