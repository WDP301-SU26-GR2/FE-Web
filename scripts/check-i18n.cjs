...
// Parity
for (const ns of ['common', 'welcome', 'auth', 'mangaka']) {
...
function scan(dir) {
  ...
  for (const f of fs.readdirSync(dir)) {
    ...
    if (stat.isDirectory()) missing.push(...scan(p));
      ...
      const nsMatch = txt.match(/useTranslation\(\s*(?:\[['"](\w+)['"]\s*,\s*['"](\w+)['"]\]|\[['"](\w+)['"]\]|['"](\w+)['"])/);
      const namespaces = nsMatch
        ? nsMatch[1] ? [nsMatch[1], nsMatch[2]] : nsMatch[3] ? [nsMatch[3]] : [nsMatch[4]]
        : ['common'];
