 ...
 */
import { readFileSync, writeFileSync, readdirSync, mkdirSync, renameSync, statSync, existsSync, rmSync } from 'node:fs'
...
function extractSchemaImportsFromFlatBarrel(filePath) {
  ...
  const re = /import\s+type\s*\{([^}]+)\}\s*from\s*['"]\.\.\/\.\.\/model['"]\s*;?/g
  const types = new Set()
  let match
...
if (schemaToTag.size === 0) {
  console.log('[organize-models] no flat-barrel imports found — operations already per-tag.')
...
if (expandedSchemaToTag.size > 0) {
  ...
  for (const tag of tagDirs) {
    ...
    for (const file of walkFiles(tagDir)) {
      ...
      content = content.replace(/from\s+['"]\.\.\/\.\.\/model['"]/g, `from '../../model/${tag}'`)
      if (content !== before) {
...
const tagFolders =
  typeof _fallbackTagFolders !== 'undefined' ? _fallbackTagFolders : [...new Set(expandedSchemaToTag.values())].sort()
...
for (const tag of tagFolders) {
  ...
  for (const typeName of usedTypes) {
    ...
    crossTagLines.push(`export type { ${typeName} } from '../${owner}/${fileBase}';`)
  }

  const localExports = localFiles.map((f) => `export * from './${f.replace(/\.ts$/, '')}'`)
